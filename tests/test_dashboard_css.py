...
def test_invoke_dialog_css_has_role_selector_layout() -> None:
    ...
    assert "max-height: 38;" in css.NEW_AGENT_CSS
...
def test_message_dialog_css_matches_notes_shell() -> None:
    ...
    assert "#agent-message-preset-grid Button {" in css.AGENT_MESSAGE_CSS


def test_preset_message_dialog_css_uses_soft_pear_green_border() -> None:
    assert "PresetMessageScreen {" in css.PRESET_MESSAGE_CSS
    assert "background: transparent;" in css.PRESET_MESSAGE_CSS
    assert "#preset-message-dialog {" in css.PRESET_MESSAGE_CSS
    assert "border: thick #9acb7a;" in css.PRESET_MESSAGE_CSS
    assert "#preset-message-template-select {" in css.PRESET_MESSAGE_CSS
    assert "#preset-message-input {" in css.PRESET_MESSAGE_CSS
    assert "#preset-message-shortcuts-hint {" in css.PRESET_MESSAGE_CSS
