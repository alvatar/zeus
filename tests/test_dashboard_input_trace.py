...
def test_zeus_uses_linux_driver_wrapper_without_trace(monkeypatch) -> None:
    monkeypatch.delenv("ZEUS_INPUT_TRACE", raising=False)
    monkeypatch.setattr(App, "get_driver_class", lambda self: LinuxDriver)
...
def test_kitty_keyboard_protocol_enabled_by_default(monkeypatch) -> None:
    ...
    assert _remap_keyboard_protocol_write("\x1b[>1u") == ("\x1b[>1u", None)
...
def test_kitty_keyboard_protocol_can_be_forced_to_legacy(monkeypatch) -> None:
    ...
    assert _remap_keyboard_protocol_write("\x1b[>1u") == ("\x1b[<u", "force_legacy")
    assert _remap_keyboard_protocol_write("plain-text") == ("plain-text", None)
