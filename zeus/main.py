...
def main():
    parser = argparse.ArgumentParser(
        prog="zeus",
        description="Monitor and manage coding Hippeis in kitty windows",
    )
    sub = parser.add_subparsers(dest="cmd")

    p_new = sub.add_parser("new", help="Launch a new tracked Hippeus window")
    p_new.add_argument("-n", "--name", required=True, help="Hippeus name")
    p_new.add_argument("-d", "--directory", help="Working directory")
    p_new.set_defaults(func=cmd_new)

    p_ls = sub.add_parser("ls", help="List tracked Hippeis windows")
    p_ls.set_defaults(func=cmd_ls)

    p_focus = sub.add_parser("focus", help="Focus a Hippeus window")
    p_focus.add_argument("id", help="Window ID or name")
    p_focus.set_defaults(func=cmd_focus)

    p_kill = sub.add_parser("kill", help="Close a Hippeus window")
