...
from .input_driver import ZeusLinuxDriver, kitty_keyboard_protocol_enabled
from .stream import (
...
class ZeusApp(App):
    ...
    BINDINGS = [
        ...
        Binding("ctrl+k", "kill_agent", "Kill Hippeus", show=False),
    ...
    def action_preset_message(self) -> None:
        if self._should_ignore_table_action():
            return
        agent = self._get_selected_agent()
        if not agent:
            self.notify("Select a Hippeus row to message", timeout=2)
            return
        from ..message_presets import load_preset_templates

        self.push_screen(PresetMessageScreen(agent, load_preset_templates()))
    ...
    def do_spawn_subagent(
        ...
        *,
        ...
        if clean_model:
            self.do_set_last_invoke_model_spec(clean_model)

        result: str | None = spawn_subagent(
            agent,
            clean_name,
            workspace=agent.workspace,
            model_spec=clean_model,
        )
        if result:
        ...
        if os.path.exists(wt_path):
            ...
            def _push_confirm() -> None:
                ...
                def _on_confirm(confirmed: bool | None) -> None:
                    ...
                    if confirmed:
                        self._replace_existing_worktree(
                            agent,
                            clean_name,
                            repo_root,
                            model_spec=clean_model,
    ...
    def action_send_interact(self) -> None:
        ...
        if self._has_modal_open():
            modal = self.screen
            if isinstance(modal, (AgentMessageScreen, PresetMessageScreen)):
                modal.action_send()
                return
