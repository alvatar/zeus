...
_SPARKLE_CHARS = "✦✧★✶✸✹❋●✴✵✱"
_BOOM_COLORS = (
    "#ff3366", "#ff6600", "#ffcc00", "#33ff66",
    "#00ccff", "#cc33ff", "#ffffff", "#ff99cc",
    "#66ff99", "#ff66cc", "#6699ff", "#ffff66",
)
...
class DopamineOverlay(Static):
    ...
    def _tick(self) -> None:
        ...
        # Expand firework bursts into particles
        ...
        # Age particles
        self._particles = [
            p for p in self._particles if p[4] > 1  # type: ignore[operator]
        ]
        ...
        # Place DOPAMINE text (appears after frame 8)
        if self._frame > 8:
            art = _DOPAMINE_ART
            ...
            ay = h // 2 - len(art) - 1
            # Color cycle
            colors_cycle = ["#ffcc00", "#ff6600", "#ff3366", "#cc33ff",
                            "#00ccff", "#33ff66"]
            ci = self._frame % len(colors_cycle)
            tc = colors_cycle[ci]
    ...
    def _fade_out(self) -> None:
        if self._tick_timer:
            ...
            self._tick_timer = None
        self.styles.animate(
            "opacity", 0.0, duration=0.8, easing="out_cubic",
        )
...
# Terminator (T-800) — classic ASCII art, large format
...
# Brightness-based color mapping for ASCII art chars
_TERM_CHAR_COLOR: dict[str, str] = {
    " ": "", ".": "#222233", ":": "#333344", "-": "#444455",
    "=": "#555566", "+": "#666677", "*": "#778899", "#": "#99aabb",
    "%": "#aabbcc", "@": "#ccddee",
}
...
class SteadyLadOverlay(Static):
    ...
    def _tick(self) -> None:
        ...
        # ── Fire: fill entire background ──
        # Intensity rises from top to bottom (0.0 → 1.0)
        f = self._frame
        for y in range(h):
            vert = y / max(1, h - 1)  # 0 at top, 1 at bottom
            for x in range(w):
                # Distance from horizontal center (0→1)
                cx = abs(x - w // 2) / max(1, w // 2)
                # Base intensity: strong at bottom & edges, lighter in center
                intensity = 0.3 + 0.5 * vert + 0.2 * cx
                # Animated wave
                wave = 0.15 * math.sin(x * 0.3 + f * 0.25 + y * 0.1)
                intensity += wave + random.uniform(-0.08, 0.08)
                intensity = max(0.0, min(1.0, intensity))
                # Skip very dim cells (near top-center) for some darkness
                if intensity < 0.25:
                    continue
                ci = min(int(intensity * (len(_FIRE_COLORS) - 1)), len(_FIRE_COLORS) - 1)
                bi = min(int(intensity * (len(_FIRE_CHARS_BOTTOM) - 1)), len(_FIRE_CHARS_BOTTOM) - 1)
                grid[(x, y)] = (_FIRE_CHARS_BOTTOM[bi], _FIRE_COLORS[ci])

        # ── Terminator (over fire) ──
        art = _TERMINATOR
