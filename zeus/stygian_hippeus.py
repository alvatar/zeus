"""Stygian Hippeus backend (tmux-native agents without kitty windows)."""
...
def stygian_agent_row_key(agent_id: str) -> str:
    """Return stable dashboard row key for Stygian agents."""
    return f"stygian:{agent_id.strip()}"


def stygian_tmux_session_name(agent_id: str) -> str:
    """Generate deterministic tmux session name from agent id."""
    clean = agent_id.strip()
    suffix = clean[:8] if clean else "agent"
    return f"stygian-{suffix}"


def is_stygian_tmux_session(session: TmuxSession) -> bool:
    """Return True when a tmux session is tagged as Stygian Hippeus."""
    return (session.backend or "").strip().lower() == STYGIAN_TMUX_BACKEND_TAG
...
def resolve_stygian_session_path(session_name: str) -> str:
    """Resolve Stygian Hippeus session path from tmux metadata."""
...
def promote_hoplite_to_stygian_hippeus(session: TmuxSession) -> tuple[bool, str]:
    """Promote a Hoplite tmux session to Stygian-Hippeus metadata."""
...
def discover_stygian_agents(
    tmux_sessions: list[TmuxSession],
    *,
    name_overrides: Mapping[str, str] | None = None,
) -> tuple[list[AgentWindow], list[TmuxSession]]:
    """Build Stygian-agent rows from tagged tmux sessions.

    Returns ``(stygian_agents, remaining_tmux_sessions)`` where remaining sessions
    exclude Stygian ones (so they aren't rendered as viewer child rows).
    """
    overrides = dict(name_overrides or {})

    stygian_by_id: dict[str, TmuxSession] = {}
    remaining: list[TmuxSession] = []

    for sess in tmux_sessions:
        if not is_stygian_tmux_session(sess):
        ...
        prev = stygian_by_id.get(sess_agent_id)
        if prev is None or sess.created > prev.created:
            stygian_by_id[sess_agent_id] = sess

    stygian_agents: list[AgentWindow] = []
    for agent_id, sess in sorted(stygian_by_id.items(), key=lambda item: item[1].name):
        row_key = stygian_agent_row_key(agent_id)
        default_name = (sess.display_name or "").strip() or f"stygian-{agent_id[:4]}"
        display_name = overrides.get(row_key, default_name)

        stygian_agents.append(
            AgentWindow(
                kitty_id=0,
                socket="",
                name=display_name,
                ...
                backend=STYGIAN_AGENT_BACKEND,
                tmux_session=sess.name,
            )
        )

    return stygian_agents, remaining


def capture_stygian_screen_text(
    session_name: str,
    *,
    full: bool = False,
    ansi: bool = False,
) -> str:
    """Capture Stygian Hippeus output from tmux pane history."""
...
def send_stygian_text(session_name: str, text: str, *, queue: bool) -> bool:
    """Send text to Stygian Hippeus (Enter or Alt+Enter queue)."""
...
def send_stygian_escape(session_name: str) -> bool:
    """Send ESC to Stygian Hippeus pane."""
    result = _run_tmux(
...
def kill_stygian_session(session_name: str) -> tuple[bool, str]:
    """Kill Stygian Hippeus tmux session."""
