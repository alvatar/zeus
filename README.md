...
### pi system-prompt appendix symlink

To use the Zeus system-prompt appendix in pi, symlink this repo file into the pi agent path:

```bash
mkdir -p ~/.pi/agent
ln -sfn "$PWD/prompts/APPEND_SYSTEM.md" ~/.pi/agent/APPEND_SYSTEM.md
```
